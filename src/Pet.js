class Pet extends Object2d {
    ...
    handleDead(){
        ...
        App.toggleGameplayControls(false, () => {
            ...
            /* 🧠 다마AI: 되살리기는 돈이 아니라 "말 걸기" — 외로움에 잠든 펫은 대화로 깨어난다 (무료·무제한) */
            App.displayConfirm(`<b>${App.petDefinition.name}</b>(이)가 외로움 속에 잠들었어요… 💤<br><small>다시 말을 걸어주면 깨어나요.</small>`, [
                {
                    name: '💬 말 걸어 깨우기',
                    onclick: () => {
                        Activities.revive();
                        if (typeof AIGrowth !== 'undefined' && AIGrowth.reviveGrace) AIGrowth.reviveGrace();
                        setTimeout(() => {
                            if (App.handlers?.dama_say) App.handlers.dama_say('…돌아왔어요! 다시는 저를 혼자 두지 마세요 🥺\n하루에 한 번은 꼭 말 걸어주기예요!', 10000, { queue: true });
                        }, 6000);
                        return false;
                    }
                },
    ...
    handleEgg(){
        ...
        App.toggleGameplayControls(false, () => {
            App.displayPopup('알 속에서 두뇌를 기다리고 있어요…<br><b>LM Studio</b>를 켜면 저절로 깨어나요!');
        })
    ...
    handleRandomSentences(){
        ...
        if(this.nextRandomSentenceTime < now){
            ...
            if(!this.nextRandomSentenceQueued){
                ...
                App.queueEvent(async () => {
                    /* 🧠 교육용 포크: 로컬 AI가 연결돼 있으면 진짜 AI가 말한다 */
                    let line = null;
                    if (typeof LocalAI !== 'undefined' && LocalAI.connected) {
                        line = await LocalAI.idleLine(me.petDefinition?.name);
                    }
                    /* 말풍선 큐(겹침 방지)로 — 대화 중이면 끝난 뒤에 말한다 */
                    if (App.handlers?.dama_say) App.handlers.dama_say(line || generateRandomSentence(), 6000, { queue: true });
                    else me.say(line || generateRandomSentence());
