        ...
        const getDialogMessage = () => {
            switch(getCareState()){
                case 'abandoned': return {
                    title: '이별',
                    msg: `벌써 <i>${App.petDefinition.name}</i>를 보내시나요? 어떤 인연은 맺어지지 못하나 봐요.<br><br>아직 준비가 안 되셨던 것 같아요.<br><br>다음 펫을 만나기 전에 한 번만 더 생각해 주세요.`,
                    sender: '펫들의 모임',
                };
                case 'leaveEarly': return {
                    title: `${App.userName}님께`,
                    msg: `<i>${App.petDefinition.name}</i>와 함께한 시간이 너무 짧았어요!<br>서로를 알기에 하루는 부족했죠.<br><br>여유가 생기면 다시 만날 수 있을 거예요.`,
                    sender: '펫들의 모임',
                }
                case 'lowCare': return {
                    msg: `잘 돌봐주지는 않으셨지만, 그래도 우리의 여정을 좋게 기억할게요. 건강하세요.`,
                    sender: App.petDefinition.name,
                }
                case 'mediumCare': return {
                    msg: `만난 지 ${moment(App.petDefinition.birthday).fromNow(true)}이 지났네요. 오르막 내리막이 있었지만, 좋았던 순간들을 가장 오래 기억할게요. 필요할 때 곁에 있어줘서 고마웠어요!`,
                    sender: App.petDefinition.name,
                }
                case 'highCare': return {
                    msg: `${moment(App.petDefinition.birthday).fromNow(true)} 동안 돌봐주셔서 고마워요. 함께한 모든 순간을 소중히 간직하고 절대 잊지 않을게요 <3`,
                    sender: App.petDefinition.name,
                }
            }
        }
