const App = {
    ...
    routes: {
        /* 🧠 다마AI: 원작 커뮤니티 링크 → Connect AI LAB 채널로 교체 */
        BLOG: 'https://github.com/wonseokjung/tamagochi-ai',
        ITCH_REVIEW: 'https://github.com/wonseokjung/tamagochi-ai',
        DISCORD: 'https://www.youtube.com/@CONNECT-AI-LAB',
    },
    async init () {
        // window load events
        ...
        /* 🧠 다마AI: 시간이 아니라 배움으로 자란다 — 시간 기반 자동 성장을 끈다 (AIGrowth.js가 성장을 맡음) */
        ...
        if(loadedData.lastTime){
            ...
            if(awaySeconds > 30 && App.ENV !== 'dev'){
                App.displayConfirm(`${App.userName ? `<b>${App.userName}</b>님, ` : ''}다시 왔네요!\n<b>${App.petDefinition.name}</b>(이)가 기다렸어요 — <b>${message}</b> 동안이나요`, [
                    {
                        name: '확인',
                        onclick: () => {}
                    }
    ...
    handleInputCode: function(rawCode){
        ...
        function showAlreadyUsed(){
            App.displayPopup(`이 코드는 한 번만 쓸 수 있어요`);
            return false;
        }
        ...
        switch(code){
            ...
            case "XUZFWQ":
                ...
                App.displayPopup(`축하해요! ${App.petDefinition.name}(이)가 $250을 받았어요!`, 5000);
                break;
            case "PRNCSS":
                if(!addEvent(codeEventId, () => {
                    App.displayPopup(`성공!`, 5000, () => {
            ...
            case "C29YCNK":
                if(!addEvent(codeEventId, () => {
                    ...
                    App.displayPopup(`불편을 드려 죄송해요! 보상으로 <b>미션 포인트 ${missionPtsAmount}</b>와 <b>$${goldAmount}</b>를 드려요!`, 5000, () => {
            ...
            case "CONNECTAILAB":
                if(!addEvent(codeEventId, () => {
                    App.displayPopup(`🎁 <b>1000코인</b>을 받았어요!<br><br>다마AI를 사랑해줘서 고마워요!`, 5000, () => {
    ...
    handleInGameEvents: function(){
        ...
        /* 🧠 다마AI: 원작의 부팅 팝업(업데이트 공지·별점 유도) 전부 삭제 — 시작은 조용하고 깨끗하게 */
    ...
    runRandomEncounters: function(){
        /* 🧠 다마AI: 원작의 랜덤 방해 이벤트 전부 끔 — 학교 초대 편지·매일 신문 배달(데일리 하이라이트)·
           유령 조우·밤 도둑. "AI 공부가 유일한 엔진"이라 학습과 무관한 인터럽트는 제거.
           복원하려면 아래 return 한 줄만 지우면 된다. */
        return;
    ...
    handlers: {
        ...
        show_rating_dialog: function(){
            return App.displayConfirm(`재밌게 즐기고 있다면 GitHub에서 <b>⭐ 별</b>을 눌러주세요! 큰 힘이 됩니다!`,
                [
                    {
        ...
        show_set_pet_name_dialog: function(text = '새 알의 이름을 지어주세요!'){
            ...
            const element = App.displayPrompt(prompt, [
                {
                    name: '결정',
                    onclick: (value) => {
                        if(!value || !value.trim()) return true; /* 이름은 필수 — 빈 값이면 창 유지 */

                        App.pet.petDefinition.name = value.trim().slice(0, 32);
                        App.save();
                        App.displayPopup(`이름이 <b>"${App.pet.petDefinition.name}"</b>(으)로 정해졌어요!`)
                        /* 🧠 다마AI: 첫 시작이면 이어서 "내 이름"도 정한다 — 펫이 주인을 이름으로 부른다 */
                        if(!App.userName) setTimeout(() => App.handlers.show_set_username_dialog(), 700);
                    }
                },
            /* 🧠 다마AI: 새 알에는 랜덤 영어 이름을 미리 채우지 않는다 — 직접 짓게 한다 */
            ], text === '새 알의 이름을 지어주세요!' ? '' : (App.pet.petDefinition.name || ''));
        ...
        show_set_username_dialog: function(){
            /* 🧠 다마AI: 맨 처음 만나는 화면 — 한글 이름 허용, 펫이 이 이름으로 불러준다 */
            ...
            App.displayPrompt(`반가워요! 👋<br>당신의 이름을 알려주세요<br><small>펫이 이 이름으로 불러줘요</small>`, [
                {
                    name: '결정',
                    onclick: (username) => {
                        username = (username || '').toString().trim();
                        if(!validate(username)) return App.displayPopup('이름은 한글·영문·숫자만 쓸 수 있어요.');
                        if(username.length < 1) return App.displayPopup('이름을 입력해 주세요.');
                        if(username.length > 12) return App.displayPopup('이름은 12자까지 가능해요.');
            ...
            return App.displayList([
                {
                    ...
                    name: `<b>${BRAND.name}</b>
                    <br>
                    <small>by</small> ${BRAND.maker}
                    <small>
                        <a href="${BRAND.youtube}" target="_blank">youtube</a>
                        <a href="${BRAND.acb}" target="_blank">aicitybuilders</a>
                        <a href="${BRAND.ezer}" target="_blank">ezerai</a>
                        <a href="${BRAND.repo}" target="_blank">github</a>
                    </small>
                    `
                },
                {
                    /* 원작 크레딧 (라이선스 의무) — 최소 표기 */
                    type: 'text',
                    name: `<small style="opacity:.55;font-size:.8em">based on <a href="${BRAND.origin.url}" target="_blank" style="color:inherit">${BRAND.origin.name}</a> © ${BRAND.origin.author} · ${BRAND.origin.license}</small>`
                },
                {
                    type: 'text',
            ...
            const settings = App.displayList([
                {
                    ...
                    _mount: (e) => {
                        e.querySelector('#emergency-backup').onclick = (evt) => {
                            ...
                            evt.target.innerHTML = `${App.getIcon('check')} 내보냄!`
                        }
                    }
                },
                {
                ...
                },
                {
                    ...
                    _mount: (e) => e.innerHTML = `알림: <i>${App.settings.notifications ? '켬' : '끔'}</i>`,
                    name: `알림`,
                    onclick: (btn) => {
                    ...
                    onclick: () => {
                        return App.displayList([
                            ...
                            },
                            {
                                ...
                                onclick: async () => {
                                    const loadingPopup = App.displayPopup('불러오는 중…');
                                    const charCode = await App.getSaveCode();
                                    loadingPopup.close();
                                    App.displayConfirm(`여기서 내 <b>저장 코드</b>를 복사해, 다른 기기에서 이어서 키울 수 있어요`, [
                                        {
                                            name: '확인',
                                            onclick: () => {
                                                App.displayConfirm(`코드를 복사한 뒤, 다른 기기에서 ${BRAND.name}를 열고 <b>설정 > 코드 입력</b>에 붙여넣으세요`, [
                                                    {
                                                        name: '확인',
                                                        onclick: () => {
                                                            try {
                                                                ...
                                                                App.displayPopup('저장 코드 복사 완료!', 1000);
                                                            } catch(e) {
                ...
                },
                {
                    ...
                    onclick: () => {
                        const display = App.displayList([
                            ...
                            {
                                name: '<label class="custom-file-upload"><input id="mod-file" type="file"></input>+ 모드 추가</label>',
                                onclick: (btn) => {
                                    return true;
                                }
                            },
                            {
                                name: `켜진 모드 (${App.mods.length})`,
                                onclick: () => {
                                    // App.displayPopup(JSON.stringify(App.mods, null, 2), 5000);
                                    if(!App.mods.length) return App.displayPopup('설치된 모드가 없어요');
                                                    ...
                                                    onclick: () => {
                                                        const modInfoScreen = App.displayList([
                                                            ...
                                                            },
                                                            {
                                                                name: '제거',
                                                                onclick: () => {
                                                                    App.displayConfirm(`<b>${modInfo.name}</b> 모드를 제거할까요?`, [
                                                                        {
                                                                            name: '네',
                                                                            onclick: () => {
                                                                                ...
                                                                                App.displayPopup(`<b>${modInfo.name}</b> 제거 완료! 새로고침…`, null, () => location.reload());
                                                                            }
                                                                        },
                                                                        {
                                                                            name: '아니요',
                        ...
                        App.handleFileLoad(display.querySelector('#mod-file'), 'readAsText', (data) => {
                            try {
                                ...
                                if(duplicateIndex != -1){
                                    ...
                                    App.displayPopup(`<b>${json.name}</b> 업데이트 완료! <br><br> 새로고침…`, 2000, () => {
                                        location.reload();
                                    })
                                } else {
                                    App.mods.push(json);
                                    App.sendAnalytics('mod_install', json.name);
                                    App.displayPopup(`<b>${json.name}</b> 설치 완료! <br><br> 새로고침…`, 2000, () => {
                                        location.reload();
                                    })
                                }
                                App.save();
                                ...
                                console.log(e);
                                App.displayPopup(`문제가 생겼어요. 잘못된 패키지: ${e}`);
                            }
                        })
                ...
                { type: 'separator', _ignore: ignoreFirstDivider },
                {
                    ...
                    onclick: () => {
                        ...
                        return App.displayList([
                            ...
                            {
                                /* 🧠 다마AI: 시간 성장 대신 배움으로 자란다 — 설정은 안내만 */
                                _mount: (e) => e.innerHTML = `🌱 성장: <i>배움으로 자람</i>`,
                                onclick: (e) => {
                                    App.displayPopup(`다마AI는 시간이 아니라 <b>배움</b>으로 자라요!<br>💬 대화하고 📚 가르치면 커요.`, 5000);
                                    return true;
                                }
                            },
                            {
                            ...
                            {
                                _mount: (e) => e.innerHTML = `${getStateIcon(App.settings.showWantName)} 원하는 것 표시: <i>${App.settings.showWantName ? '켬' : '끔'}</i>`,
                                onclick: (item) => {
                            ...
                            {
                                _mount: (e) => e.innerHTML = `${getStateIcon(App.settings.genderedPets)} gendered pets: <i>${App.settings.genderedPets ? '켬' : '끔'}</i>`,
                                onclick: (item) => {
                            ...
                            },
                            {
                                ...
                                <div class="overflow-hidden flex">
                                    <div class="marquee">
                                        skills affecting evolution: <i>${App.settings.skillsAffectingEvolution ? '켬' : '끔'}</i>
                                    </div>
                                </div>
                                `,
                                onclick: (item) => {
                ...
                },
                {
                    ...
                    onclick: () => {
                        App.displayList([
                            {
                                name: `효과음: <i>${App.settings.playSound ? '켬' : '끔'}</i>`,
                                onclick: (item) => {
                                    App.settings.playSound = !App.settings.playSound;
                                    item.innerHTML = `효과음: <i>${App.settings.playSound ? '켬' : '끔'}</i>`;  
                                    return true;
                                }
                            },
                            {
                                _mount: (e) => e.innerHTML = `배경음악: <i>${App.settings.playMusic ? '켬' : '끔'}</i>`,
                                onclick: (item) => {
                            ...
                            {
                                name: `진동: <i>${App.settings.vibrate ? '켬' : '끔'}</i>`,
                                onclick: (item) => {
                                    App.settings.vibrate = !App.settings.vibrate;
                                    item.innerHTML = `진동: <i>${App.settings.vibrate ? '켬' : '끔'}</i>`;  
                                    return true;
                                }
                            },
                            {
                                _mount: (e) => e.innerHTML = `클래식 메뉴: <i>${App.settings.classicMainMenuUI ? '켬' : '끔'}</i>`,
                                onclick: (item) => {
                            ...
                            {
                                _mount: (e) => e.innerHTML = `터치 효과: <i>${App.settings.tapEffect ? '켬' : '끔'}</i>`,
                                onclick: (item) => {
                ...
                },
                {
                    ...
                    name: '<b>유튜브 <span style="color:#ff4444;text-shadow:none;">구독</span></b>',
                    onclick: () => true,
                },
                { type: 'separator' },
                {
        ...
        open_food_list: function(props = {}){
            ...
            for(let food of Object.keys(App.definitions.food)){
                ...
                list.push({
                    ...
                    price,
                    icon: foodIcon,
                    shortName: `<div class="icon">${foodIcon}</div> <span class="ellipsis">${koFood(food)}</span>`,
                    name: `
                        ${foodIcon} 
            ...
            let acceptLabel = '먹이기';
            if(buyMode) acceptLabel = '구매';
            else if(sellMode) acceptLabel = '팔기';
        ...
        open_feeding_menu: function(){
            App.displayList([
                ...
                {
                    /* 🧠 다마AI: 코인 쓰는 곳 — 수다 코인으로 장을 본다 (숨긴 마켓의 구매 메뉴 재사용) */
                    name: `🛒 장보기 <b>($${App.pet.stats.gold})</b>`,
                    onclick: () => {
        ...
        open_stats_menu: function(){
            ...
            App.displayList([
                ...
                {
                    name: `업적 ${hasNewlyUnlockedAchievements ? App.getBadge('Rewards!') : ''}`,
                    onclick: () => {
                ...
                {
                    name: App.settings.genderedPets ? '이름·성별 바꾸기' : '이름 바꾸기',
                    onclick: () => {
                        const prompt = App.settings.genderedPets ? '이름과 성별을 알려주세요:' : '펫의 새 이름을 알려주세요:';
        ...
        open_profile: function(){
            ...
            content.innerHTML = `
                <div data-fb-focusable class="flex-center flex-1 flex flex-gap-05 inner-padding surface-stylized height-auto relative">
                    ...
                    <span>
                        태어난 지 ${moment(App.petDefinition.birthday).fromNow(true)}
                    </span>
                </div>
                <div class="surface-stylized inner-padding flex flex-gap-05 flex-dir-col">
                    ...
                    <div data-fb-focusable class="relative mt-6 width-full">
                        <div class="stats-label left-0">경험치</div>
                    ...
                    <div data-fb-focusable class="relative mt-6 width-full">
                        <div class="stats-label left-0">성격</div>
                        <div class="pet-trait-icons-container">
                    ...
                    <div data-fb-focusable class="relative mt-6 width-full">
                        <div class="stats-label left-0">별자리</div>
                        <div class="pet-trait-icons-container">
        ...
        open_craftables_list: function(){
            ...
            sliderInstance = App.displaySlider(list, null, {accept: '만들기'});
            return sliderInstance;
        ...
        open_room_background_list: function(onlyFurnishables, filterFn){
            ...
            list = list.sort((a, b) => b.isNew - a.isNew)
            sliderInstance = App.displaySlider(list, null, {accept: '구매'}, `$${App.pet.stats.gold + (salesDay ? ` <span class="sales-notice">DISCOUNT DAY!</span>` : '')}`);
            return sliderInstance;
        },
        open_shell_background_list: function(){
            ...
            sliderInstance = App.displaySlider(list, null, {accept: '결정'});
            return sliderInstance;
        },
        open_background_pattern_list: function(){
            ...
            sliderInstance = App.displaySlider(list, null, {accept: '전환'});
            return sliderInstance;
        },
        ...
        open_market_menu: function(){
            ...
            App.displayList([
                {
                    name: `🍚 밥 사기 ${hasNewFood ? App.getBadge('new!') : ''}`,
                    onclick: () => {
                ...
                {
                    name: `🍭 간식 사기 ${hasNewTreat ? App.getBadge('new!') : ''}`,
                    onclick: () => {
                ...
                {
                    name: `💊 약 사기 ${hasNewMed ? App.getBadge('new!') : ''}`,
                    onclick: () => {
    ...
    displaySlider: function(listItems, activeIndex, options, additionalText){
        ...
        acceptBtn.innerHTML = options?.accept || '확인';
    ...
    takeScreenshot: () => {
        ...
        const name = BRAND.nameEn + '_' + moment().format('D-M-YY_h-m-s');
    ...
    /* 🧠 다마AI: 원작의 외부 전송(RudderStack·구글폼) 전부 제거 — 어떤 데이터도 밖으로 나가지 않는다 */
    ...
    sendErrorLog: function(error, force){
        if(error) console.error('[다마AI]', error);
    },
    ...
    apiService: {
        /* 🧠 다마AI: 원작의 온라인 서버(펫 업로드·교류) 연결 차단 — 100% 로컬 약속.
           sendRequest는 항상 오프라인 에러로 응답해서 기존 호출부가 안전하게 처리한다. */
        _getUid: () => {
            return App.userName + '-' + App.userId;
        },
        sendRequest: async (params, handler) => {
            const offline = { error: 'offline (다마AI는 외부 서버에 연결하지 않아요)' };
            const handledResult = handler?.(offline, true);
            return handledResult ?? offline;
        },
        addPetDef: (petDef) => {
...
window.addEventListener('beforeinstallprompt', (e) => {
    ...
    const showInstallPrompt = () => {
        App.addEvent(`pwa_install_notice_01`, () => {
            App.displayConfirm(`<b>${BRAND.name}</b>를 앱으로 설치할까요?`, [
                {
                    name: 'install',
                    onclick: () => {
                        App.installAsPWA();
